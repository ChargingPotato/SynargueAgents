    ...
    <div class="score-card card">
      <div class="score-row">
        <div class="score-side side-a">
          ...
          <span class="score-label-text">{{ state.sides?.side_a || '正方' }}</span>
        </div>
        ...
        <div class="score-side side-b">
          ...
          <span class="score-label-text">{{ state.sides?.side_b || '反方' }}</span>
        </div>
      </div>
...
const scoreA = computed(() => {
  const s = props.state?.tendency_score?.side_a
  return Math.round((s || 0.5) * 100)
})

const scoreB = computed(() => {
  const s = props.state?.tendency_score?.side_b
  return Math.round((s || 0.5) * 100)
})
</script>
