<template>
  <div class="research-review fade-in">
    ...
    <h2 class="section-title">
      📌 辩题：<span class="topic-highlight">{{ store.topic || '加载中...' }}</span>
    </h2>
    ...
    <div class="review-grid">
      ...
      <div class="side-panel side-a">
        <div class="side-header">
          ...
          <strong>{{ store.sides.side_a || '正方观点' }}</strong>
        </div>
          ...
          >
            <label class="item-checkbox">
              ...
              <div class="item-content">
                ...
                <MarkdownContent class="item-text" :content="item.content" />
      ...
      <div class="side-panel side-b">
        <div class="side-header">
          ...
          <strong>{{ store.sides.side_b || '反方观点' }}</strong>
        </div>
        <div class="items-list">
          <div
    ...
    <div class="submit-area">
      ...
      >
        ...
        {{ store.loading ? '处理中...' : '✅ 确认资料库，生成一级立论与交叉反驳' }}
      </button>
    </div>
  </div>
</template>
...
function syncState() {
  localDataA.value = (store.state.research_data_a || []).map(item => ({
    ...item,
    valid: item.valid !== false,
  }))
  localDataB.value = (store.state.research_data_b || []).map(item => ({
    ...item,
    valid: item.valid !== false,
  }))
}

watch(() => store.state, syncState, { immediate: true, deep: true })

function handleSubmit() {
  store.submitReview(localDataA.value, localDataB.value)
}
</script>
